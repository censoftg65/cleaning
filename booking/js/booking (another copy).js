...
$(document).ready(function() {
    ...
    function fun_bookingCalulator(bedroom, price, filter_type) {
        var hours_options = $("#txtServiceHours");
        ...
        if (bedroom !== null || bedroom !== "") {
            $.each(cal_data[bedroom], function(index, jsonObject) {
                $.each(jsonObject, function(index, value) {
                    ...
                    if (filter_type == 'extra_service') {
                       hours_options.append(new Option(index, index));
                        prices_options.append(new Option(Number(value + price).toFixed(2), Number(value + price).toFixed(2)));
                    }
                    if (filter_type == 'service_hours') {
                         alert(index);
                        hours_options.append(new Option(index, index));
                        //prices_options.append(new Option(Number(value * price).toFixed(2), Number(value * price).toFixed(2)));
                    }
    ...
    $("#txtBedroom").change(function() {
        filter_type = 'bedroom';
        bedroom = $(this).val();
        fun_bookingCalulator(bedroom, default_service_price, filter_type);
    });

    /*Change Total On change Hours Selection*/
    $("#txtServiceHours").change(function() {
        filter_type = 'service_hours';
        price_by_hours = 30;
        fun_bookingCalulator(bedroom, price_by_hours, filter_type);
    });

    /*On click extra services*/
    $("input[type=checkbox]").click(function() {
        filter_type = 'extra_service';
        if ($(this).prop("checked") === true) {
        ...
        fun_bookingCalulator(bedroom, service_price, filter_type);
    });
})